...
def claim(user_data, index):
    headers = {
        ...
        'origin': 'qfan.qcloud.asia',
        'pragma': 'no-cache',
        'referer': 'https://qfan.qcloud.asia/',
    ...
    try:
     # Claim
        print(Fore.YELLOW + Style.BRIGHT + "[ Claim ] : Trying to claim. ", end="", flush=True)
            claim_response = requests.post('https://qfan-api.qcloud.asia/api/qpoint-transaction/claimMissionReward', headers=headers)
...
def auto_clear_task(user_data):
    ...
    headers['--webapp-init'] = user_data
    actions = [
        {"quest_id": "missions", "action": "follow_twitter"},
        
    ]
    for action in actions:
        response = requests.post('https://qfan-api.qcloud.asia/api/missions', headers=headers, json=action)
    ...
    response = requests.post('https://qfan-api.qcloud.asia/api/qpoint-transaction/claimMissionReward', headers=headers, json={"quest_id": "missions"})
    if response.status_code == 200:
